...
# 사용한 제스쳐
gesture = {0: 'fist', 1: 'one', 3: 'three', 5: 'five',
           7: 'rock', 9: 'yeah', 10: 'ok'}
...
while cap.isOpened():
        ...
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if len(rps_result) >= 1:
                ...
                if rps_result[0]['rps'] == 'fist':
                    ...
                    frame = overlay_transparent(frame,
                                                overlay3,
                                                int(hand_landmarks.landmark[3].x * width - 80),
                                                int(hand_landmarks.landmark[3].y * height - 25),
                                                overlay_size=(250, 250))

                    frame = overlay_transparent(frame,
                                                overlay4, int(640/2), int(480/2), overlay_size=(300, 300))

                elif rps_result[0]['rps'] == 'rock':
                    text = 'gesture : rock'
                    cv2.putText(frame,
                                text=text,
                                org=(rps_result[0]['org'][0], rps_result[0]['org'][1] + 70),
    ...
    # cv2.putText(final_frame, str(int(fps)) + " FPS", (10, 70), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)

    # Display the resulting image
    cv2.imshow("Face Mesh", frame)
    # cv2.imshow("Face Mesh", cv2.flip(frame, 1))
    # cv2.imshow("Face Mesh", cv2.flip(final_frame, 1))
